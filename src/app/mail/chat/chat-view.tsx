...
export function ChatView() {
  ...
  const send = (text: string) => {
    ...
    setInput("");
    start(async () => {
      const r = await chatTurnAction(history, message);
      if (!r.ok) {
        toast.error(r.error);
        setTurns((t) => [...t, { role: "assistant", content: `出错了：${r.error}` }]);
        return;
      }
      setTurns((t) => [...t, { role: "assistant", content: r.data.reply, proposals: r.data.proposals, trace: r.data.trace, model: r.data.model }]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="mb-2">
        ...
        <p className="text-xs text-muted-foreground">AI 会先检索你的邮件再回答；涉及归档、删除等操作时只会给出建议，由你确认后执行。</p>
      </div>
            ...
            <div className={cn("max-w-[85%] space-y-2 rounded-lg px-3 py-2 text-sm", t.role === "user" ? "bg-primary text-primary-foreground" : "bg-muted")}>
                  ...
                  {t.proposals.map((p, j) => {
                    ...
                    return (
                      <div key={key} className="flex flex-wrap items-center gap-2 text-xs">
                        <span className="rounded bg-muted px-1">{ACTION_LABELS[p.action]}{p.value ? ` ${p.value}` : ""}</span>
              ...
              {t.trace?.length ? (
                <details className="text-xs text-muted-foreground">
                  <summary className="cursor-pointer">查看检索过程（{t.trace.length} 步{t.model ? ` · ${t.model}` : ""}）</summary>
      ...
      <p className="mt-1 text-[11px] text-muted-foreground">
        找不到想要的邮件？先到 <Link href="/mail/settings/ai" className="underline">AI 设置</Link> 配置 embedding 模型以启用语义搜索。
      </p>
    </div>
  );
}
